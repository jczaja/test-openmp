...
#include <x86intrin.h>
#include<kernels/dnnl_fp32_conv_winograd_kernel.hpp>

REGISTER_KERNEL(DNNLConvWinogradKernel);

DNNLConvWinogradKernel::DNNLConvWinogradKernel() :
DNNLKernel(false)
{
  // Register kernel
  kernels[std::string("dnnl_winograd_conv")] = this;
}

void DNNLConvWinogradKernel::Init(platform_info &pi, int n, int c, int h, int w)
{
  tsc_ghz_ = pi.tsc_ghz;
  ...
  auto conv_desc = dnnl::convolution_forward::desc(dnnl::prop_kind::forward_inference,
           dnnl::algorithm::convolution_winograd, src_md, weights_md, bias_md, dst_md, strides, padding, padding);
...
void DNNLConvWinogradKernel::ShowInfo(bool cold_caches)
{
  ...
  std::cout << std::endl << " DNNL Winograd Conv " << n << "x" << c << "x" 
