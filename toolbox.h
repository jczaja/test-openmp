...
class XeonMemoryTraffic : public MemoryTraffic 
{
 ...
 count_cas_write3_(0), count_cas_write4_(0), count_cas_write5_(0)
 {
    ...
    pe_.type = UNCORE_IMC_0_TYPE; // Intel Xeon cas_count_read and cas_count_write goes from 11 , 12 or 13 depending on platform 
    fdr_ = perf_event_open(&pe_, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);
    pe_.type = UNCORE_IMC_0_TYPE + 1; // Intel Xeon cas_count_read IMC 1
    fdr1_ = perf_event_open(&pe_, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);
    pe_.type = UNCORE_IMC_0_TYPE + 2; // Intel Xeon cas_count_read IMC 2
    fdr2_ = perf_event_open(&pe_, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);
    pe_.type = UNCORE_IMC_0_TYPE + 3; // Intel Xeon cas_count_read IMC 3
    fdr3_ = perf_event_open(&pe_, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);
    pe_.type = UNCORE_IMC_0_TYPE + 4; // Intel Xeon cas_count_read IMC 4
    fdr4_ = perf_event_open(&pe_, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);
    pe_.type = UNCORE_IMC_0_TYPE + 5; // Intel Xeon cas_count_read IMC 5
    fdr5_ = perf_event_open(&pe_, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);

    // Now modify config to have data_writes counted    
    pe_.config = 3076;
    pe_.type = UNCORE_IMC_0_TYPE;
    fdw_ = perf_event_open(&pe_, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);
    pe_.type = UNCORE_IMC_0_TYPE + 1; // Intel Xeon cas_count_write IMC 1
    fdw1_ = perf_event_open(&pe_, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);
    pe_.type = UNCORE_IMC_0_TYPE + 2; // Intel Xeon cas_count_write IMC 2
    fdw2_ = perf_event_open(&pe_, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);
    pe_.type = UNCORE_IMC_0_TYPE + 3; // Intel Xeon cas_count_write IMC 3
    fdw3_ = perf_event_open(&pe_, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);
    pe_.type = UNCORE_IMC_0_TYPE + 4; // Intel Xeon cas_count_write IMC 4
    fdw4_ = perf_event_open(&pe_, -1, 0, -1, PERF_FLAG_FD_CLOEXEC);
    pe_.type = UNCORE_IMC_0_TYPE + 5; // Intel Xeon cas_count_write IMC 5
